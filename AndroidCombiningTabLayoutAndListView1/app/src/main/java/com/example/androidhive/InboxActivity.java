...
public class InboxActivity extends ListActivity {
	...
	{
		Toast.makeText(getApplicationContext(), "xxxxx" + inboxList.get(position).get(TAG_SUBJECT),
				Toast.LENGTH_LONG).show();
		Intent myIntent = new Intent(this, InboxDetailActivity.class);
		myIntent.putExtra(TAG_SUBJECT, inboxList.get(position).get(TAG_SUBJECT));
	...
	class LoadInbox extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
            ...
            paramx.add(new BasicNameValuePair("ZUMO-API-VERSION", "2.0.0"));
            JSONArray jsonx = jsonParser.makeHttpRequestx("http://shimizuappservice.azurewebsites.net/tables/Device", "GET",
                    paramx);
            ...
            if(json == null)
            {
                ...
                //Log.d("Inbox JSON: ", jsonx.toString());
            }
            ...
            try {
                ...
                for (int i = 0; i < jsonx.length(); i++) {
                    JSONObject c = jsonx.getJSONObject(i);
                    // Storing each json item in variable
                    String name = c.getString("name");
                    Log.d("#DEBUG JSON: ", name);
                }
